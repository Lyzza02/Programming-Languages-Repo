...
map<string, string> Make_Regex_Map() {
  map<string, string> my_map{
      {"\\;|\\{|\\}|\\(|\\)|\\,|\\#", "Special Symbol"},
      {"int|char|double|float|bool|main|using|namespace|std|return", "Keywords"},
      ...
      {"[+-]?[0-9][.][0-9][f]+", "Float"},
      {"[+-]?[0-9][.][0-9]+", "Double"},
      {"[^include][^iostream][^int][^main][^cin][^cout][^;][^>>][^,][^[B "
       ";cin]][a-z]+",
       "Identifier"},
      {"[a-z]+", "Variable"},
      {"[ ]", ""},
...
map<size_t, pair<string, string>> Match_Language(map<string, string> patterns,
                                                 string str) {
...
int main() {
  ...
  if (fin.is_open()) {
    ...
    patterns from the ['Source Code':'Defined Pattern' via a Regex named
    'compare'. */
    map<size_t, pair<string, string>> lang_matches =
        Match_Language(patterns, str);
    ...
    for (auto match = lang_matches.begin(); match != lang_matches.end();
      ...
      if (!(match->second.first == " ") && !(match->second.first == "//")) {
        if (match->second.second == "Variable" ||
            match->second.second == "Identifier") {
          cout.width(40);
          if (count < 10) {
    ...
    while (command != "EXIT") {
      ...
      cout << "\n\n\t PRESS TYPE `EXIT` TO CLOSE WINDOW.\n\t NOTE: AN OUTPUT "
              "FILE WILL BE GENERATED IN THE SAME FOLDER AS `Output.txt` \n";
      cin.width(40);
      cin >> command;
